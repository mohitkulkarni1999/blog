...
const CRON_SCHEDULE = process.env.AI_BLOGGER_CRON || '30 2 * * *';
const POSTS_PER_DAY = parseInt(process.env.AI_BLOGGER_POSTS || '2', 10);
...
function startAIBloggerScheduler() {
    ...
    console.log(`[Scheduler] 🤖 AI Blogger scheduled — ${POSTS_PER_DAY} posts/day at cron: "${CRON_SCHEDULE}"`);

    cron.schedule(CRON_SCHEDULE, async () => {
        console.log('[Scheduler] ⏰ AI Blogger cron triggered...');
        try {
            await runAIBlogger(POSTS_PER_DAY);
        } catch (err) {
